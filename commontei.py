...
def write_and_clean(urn, lang, parsed, citations,target):
    ...
    """
    body = parsed.xpath("//tei:body", namespaces=ns)[0]
    # Get its children
    ...
    # Find encodingDesc
    encodingDesc = parsed.xpath("//tei:encodingDesc", namespaces=ns)[0]
    encodingDesc.append(refsDecl)

    """
        Search for old //encodingDesc/refsDecl and refsDecl/state and correct them
    """
    refsDecls = parsed.xpath("//tei:encodingDesc/refsDecl[@doctype]", namespaces=ns)
    for refsDecl in refsDecls:
        refsDecl.set("n", refsDecl.get("doctype"))
        del refsDecl.attrib["doctype"]

    states = parsed.xpath("//tei:encodingDesc/refsDecl/state", namespaces=ns)
    for state in states:
        state.tag = "refState"
    ...
    """
    languages = parsed.xpath("//tei:langUsage/language[@id]", namespaces=ns) + parsed.xpath("//langUsage/lang[@id]", namespaces=ns)
    ...
    """
    pbs = parsed.xpath("//tei:pb[@id]", namespaces=ns)
    for pb in pbs:
    ...
    """
        Clean keyboarding/p
    """
    ps = parsed.xpath("//tei:sourceDesc/p", namespaces=ns)
    for p in ps:
    ...
    """
    body_text = parsed.xpath("//tei:body", namespaces=ns) + parsed.xpath("//text", namespaces=ns)
    ...
    """
        Clear refsDecl/step
    """
    refsdecls_step = parsed.xpath("//tei:refsDecl/step/parent::refsDecl", namespaces=ns)
    ...
    """
        Fix anchored
    """
    anchoreds = parsed.xpath("//tei:*[@anchored='yes']", namespaces=ns)
